...
const Bookmarks = () => {
    ...
    const fetchBookmarkedLocations = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL}/api/user/location/getall`
        );
        setBookmarkedLocations(response.data);
      } catch (error) {
  ...
  const updateWeatherData = async (location) => {
    try {
      const response = await axios.get(
        `${BASE_API_URL}?q=${location.city}&appid=${API_KEY}`
      );
      const weatherData = response.data;
      setBookmarkedLocations((prevLocations) =>
        prevLocations.map((prevLocation) =>
          prevLocation.city === location.city
            ? { ...prevLocation, weatherData }
            : prevLocation
        )
      );
    } catch (error) {
  ...
  const handleClick = (location) => {
    ...
    if (!location.weatherData) {
      // If not available, fetch weather data for the location
  ...
  return (
    <>
      {user ? (
        <div>
          <div className="text-2xl mb-10 px-4 py-3 gap-20 w-screen bg-[#F3F4F6] flex">
            <button
              type="button"
              className="w-full flex items-center justify-center w-1/2 px-5 py-2 text-sm  transition-colors duration-200  border-gray-700 rounded-lg gap-x-2 sm:w-auto hover:bg-gray-800 bg-gray-900 text-gray-200 "
            >
              <svg
            ...
            <h1 className="font-bold">Bookmarked Locations</h1>
          </div>
                ...
                <div className="max-w-md p-8 mx-auto rounded-lg bg-black text-white ">
                  <div className="flex justify-between space-x-8">
                    ...
                    {location.weatherData &&
                      location.weatherData.main && ( // Check if weatherData and main exist
                        <span className="font-bold text-7xl">
                          {(location.weatherData.main.temp - 273.15).toFixed(2)}
                          °C
                        </span>
                      )}
                  </div>
